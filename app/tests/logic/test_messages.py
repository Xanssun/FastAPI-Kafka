...
from logic.exception.messages import ChatWithThatTitleAlreadyExistsException
...
async def test_create_chat_command_title_already_exists(
    ...
    title_chat = faker.text()
    chat = Chat(title=Title(title_chat))
    await chat_repository.add_chat(chat)
    
    with pytest.raises(ChatWithThatTitleAlreadyExistsException):
        await mediator.handle_command(CreateChatCommand(title=title_chat))
